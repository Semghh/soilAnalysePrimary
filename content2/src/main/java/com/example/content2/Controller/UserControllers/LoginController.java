...
public class LoginController {
    ...
    public Result Login(@RequestBody HashMap params,HttpServletRequest request,HttpServletResponse response) throws IOException {
        ...
        LoginBox loginBox = new LoginBox(usersService, log, tokenMapping);
        return loginBox.methodHandler(cookieLogin,rememberMe,params,request,response);

    }


    //为了并发，提取出来的内部类，用于执行login具体操作。
    static class LoginBox {
        ...
        public LoginBox(UsersService usersService, Log log, ConcurrentHashMap<String, TokenStore> tokenMapping) {
            this.usersService = usersService;
            this.log = log;
            this.tokenMapping = tokenMapping;
        }
